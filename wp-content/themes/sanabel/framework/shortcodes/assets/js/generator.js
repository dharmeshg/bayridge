	...
	$choice.on('click', function (e) {
		...
		$.ajax({
			...
			success: function (data) {
				...
				// Init icon pickers
				$('#su-generator-settings').each(function () {
					var $generatorSettings = $(this);
					//Divider
					if ( $('.divider.style select').val() == 'solid-line' || $('.divider.style select').val() == 'dotted-line' || $('.divider.style select').val() == 'linear-gradient' ) {
						$('.divider.width').slideDown();
						$('.divider.height').slideDown();
						$('.divider.position').slideDown();
						$('.divider.color').slideDown();
					} else {
						$('.divider.width').slideUp();
						$('.divider.height').slideUp();
						$('.divider.position').slideUp();
						$('.divider.color').slideUp();
					}
					if ( $('.custom.font.style select').val() == 'style-1' || $('.custom.font.style select').val() == 'style-2' ) {
						$('.custom.font.borders_color').slideDown();
					} else {
						$('.custom.font.borders_color').slideUp();
					}
					$('.divider #su-generator-attr-style').change(function () {
						if ( $('.divider.style select').val() == 'solid-line' || $('.divider.style select').val() == 'dotted-line' || $('.divider.style select').val() == 'linear-gradient' ) {
							$('.divider.width').slideDown('300');
							$('.divider.height').slideDown('300');
							$('.divider.position').slideDown('300');
							$('.divider.color').slideDown('300');
						} else {
							$('.divider.width').slideUp('300');
							$('.divider.height').slideUp('300');
							$('.divider.position').slideUp('300');
							$('.divider.color').slideUp('300');
						}
					});
					$('.custom.font #su-generator-attr-style').change(function () {
						if ( $('.custom.font.style select').val() == 'style-1' || $('.custom.font.style select').val() == 'style-2' ) {
							$('.custom.font.borders_color').slideDown('300');
						} else {
							$('.custom.font.borders_color').slideUp('300');
						}
					});
				});
				// Init switches
