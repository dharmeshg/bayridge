		...
		foreach ($options as $value) {
			...
			switch ( $value['type'] ) {
				...
				case "button":
					...
					$img_scr = $value['image'];
					if ($img_scr) {
						$output .= '<img src="' . $img_scr . '" class="demo_preview_image" />';
					}
				...
				case 'googletypography':
					...
					if(isset($typography_stored['size'])) {
						...
						$output .= '<select class="of-typography of-typography-size select" name="'.$value['id'].'[size]" id="'. $value['id'].'_size">';
							for ($i = 0; $i < 70; $i++){ 
								$test = $i.'px';
								if ($i == 0) {$text = "Default";}else{$text = $i .'px';}
								$output .= '<option value="'. $i .'px" ' . selected($typography_stored['size'], $test, false) . '>'. $text .'</option>'; 
								}
					...
					if(isset($typography_stored['height'])) {
						...
						$output .= '<select class="of-typography of-typography-height select" name="'.$value['id'].'[height]" id="'. $value['id'].'_height">';
							for ($i = 0; $i < 120; $i++){ 
								$test = $i.'px';
								if ($i == 0) {$text = "Default";}else{$text = $i .'px';}
								$output .= '<option value="'. $i .'px" ' . selected($typography_stored['height'], $test, false) . '>'. $text.'</option>'; 
								}
